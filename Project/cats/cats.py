...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def match(para):
        split_para = split(para)
        for word1 in split_para:
            word1 = remove_punctuation(word1)
            for word2 in subject:
                if lower(word1) == lower(word2):
                    return True
        return False
    return match
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    "*** YOUR CODE HERE ***"
    minn = limit + 1
    ans_string = typed_word
    for word in word_list:
        if word == typed_word:
            return word
        dist = diff_function(typed_word, word, limit)
        if dist < minn and dist <= limit:
            ans_string = word
            minn = dist
    return ans_string
    # END PROBLEM 5
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        minn, id = 1000000000000000000, 0
        for j in player_indices:
            if times[j][i] < minn:
                id = j
                minn = times[j][i]
        ans[id].append(words[i])
    return ans
    # END PROBLEM 10
