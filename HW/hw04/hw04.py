...
def balanced(m):
    ...
    if is_planet(m):
        ...
        massl = total_mass(end(left(m)))
        massr = total_mass(end(right(m)))
        lenl = length(left(m))
        lenr = length(right(m))
        if massl * lenl == massr * lenr and balanced(end(left(m))) and balanced(end(right(m))):
            return True
        else:
            return False
